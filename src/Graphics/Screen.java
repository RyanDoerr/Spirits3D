...
public class Screen {
    ...
    public Screen(int[][] m, int mapW, int mapH, int w, int h, ArrayList tex) {
        map = m;
        mapWidth = mapW;
        mapHeight = mapH;
        //textures = tex;
    ...
    //this clears the screen, and predraws our ceiling color and floor color
        clear(pixels);
        ...
        /**
            ...
            for (int y = drawStart; y < drawEnd; y++){
                int texY = (((y*2 - height + lineHeight) << 6) / lineHeight) / 2;
                int color;
                if (side == 0) color = texture.pixels[texX + (texY * textures.get(texNum).SIZE)];
                else color = (textures.get(texNum).pixels[texX + (texY * textures.get(texNum).SIZE)]>>1) & 8355711;//make y sides darker
                pixels[x + y*(width)] = color;
            }
        }
    ...
    //draws floor and ceiling, and clears our pixels
